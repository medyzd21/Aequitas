        ...
        rx.accordion.root(
            rx.accordion.item(
                ...
                content=rx.vstack(
                    ...
                    rx.select(
                        ["healthy", "stress", "governance", "fragile"],
    ...
    return rx.vstack(
        ...
        rx.hstack(
            ...
            _panel(
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        rx.recharts.bar_chart(
                            rx.recharts.bar(data_key="mwr_balanced", name="Money’s Worth Ratio · balanced", stack_id="mwr", fill=PALETTE["good"]),
                            rx.recharts.bar(data_key="mwr_watch", name="Money’s Worth Ratio · watch", stack_id="mwr", fill=PALETTE["warn"]),
                            rx.recharts.bar(data_key="mwr_stress", name="Money’s Worth Ratio · stress", stack_id="mwr", fill=PALETTE["bad"]),
                            rx.recharts.x_axis(data_key="cohort_label", stroke=PALETTE["muted"]),
                            rx.recharts.y_axis(stroke=PALETTE["muted"], domain=[0, 1.6]),
                            rx.recharts.reference_line(
                                y=1,
                                label="Actuarial parity",
                        ...
                        simple_table(
                            [
                                ("cohort_label", "Cohort"),
                                ("mwr_display", "Money’s Worth Ratio"),
                                ("fairness_status", "Fairness status"),
                                ("fairness_status_detail", "Meaning"),
                            ],
                            AppState.twin_v2_focus_cohort_rows,
                        ),
                        rx.text(
                            "MWR = EPV benefits / EPV contributions. Around 1.00 means the cohort receives actuarially fair value.",
                            style={"color": PALETTE["text"], "font_size": "11px", "line_height": "1.55"},
                        ),
                ...
                subtitle="Representative latest-year cohorts only, colored by whether their MWR is near actuarial parity.",
            ),
            spacing="3",
            width="100%",
            align="stretch",
...
def _fund_tab() -> rx.Component:
    return rx.vstack(
        ...
        _panel(
            ...
            rx.accordion.root(
                rx.accordion.item(
                    header=rx.text(
                        ...
                        style={"color": PALETTE["text"], "font_size": "12px", "font_weight": "600"},
                    ),
                    content=rx.cond(
                        AppState.twin_v2_calibration_rows.length() > 0,
                        simple_table(
                            [("metric", "Metric"), ("value", "Value")],
                            AppState.twin_v2_calibration_rows,
                        ),
                        _empty("Run the simulation to see the starting active/retired mix, funded ratio, contribution rate, annuity factor, reserve level, and guardrail posture."),
                    ),
                ),
                collapsible=True,
                type="single",
            ...
            subtitle="A quick sanity check for whether the run starts from a healthy, stressed, or fragile pension society.",
        ),
        rx.hstack(
            _panel(
                ...
                rx.vstack(
                    ...
                    rx.hstack(
                        ...
                        _panel(
                            ...
                            rx.recharts.line_chart(
                                ...
                                rx.recharts.x_axis(data_key="year", stroke=PALETTE["muted"]),
                                rx.recharts.y_axis(stroke=PALETTE["muted"]),
                                rx.recharts.reference_line(y=0, stroke=PALETTE["muted"], stroke_dasharray="4 4"),
                                rx.recharts.cartesian_grid(stroke=PALETTE["edge"]),
                                rx.recharts.legend(),
                                rx.recharts.graphing_tooltip(),
                                data=AppState.twin_v2_annual_rows,
                            ...
                            subtitle="A positive value means the active basis is heavier than the prior; negative means lighter.",
                        ),
                        spacing="3",
                        width="100%",
                        align="stretch",
                    ...
                    rx.hstack(
                        _panel(
                            ...
                            subtitle="Shown on its own £m axis so liability scale cannot flatten percentage series.",
                        ),
                        _panel(
                            "Funded ratio (%)",
                            rx.recharts.line_chart(
                            ...
                            subtitle="Shown as a percentage so funding health remains readable next to the liability view.",
                        ),
                        spacing="3",
                        width="100%",
                        align="stretch",
    ...
    return rx.vstack(
        _panel(
            ...
            rx.cond(
                ...
                rx.vstack(
                    rx.hstack(
                        rx.vstack(
                            rx.text("Overall verdict", style={"color": PALETTE["muted"], "font_size": "10px", "letter_spacing": "0.08em", "text_transform": "uppercase"}),
                            rx.hstack(
                                rx.text(AppState.twin_v2_fairness_verdict, style={"color": PALETTE["text"], "font_size": "30px", "font_weight": "800", "line_height": "1.0"}),
                                rx.match(
                                    AppState.twin_v2_fairness_verdict_pill,
                                    ("good", pill("PASS", "good")),
                                    ("warn", pill("WATCH", "warn")),
                                    ("bad", pill("FAIL", "bad")),
                                    pill("NO RUN", "muted"),
                            ...
                            rx.text(AppState.twin_v2_fairness_reason, style={"color": PALETTE["text"], "font_size": "13px", "line_height": "1.6"}),
                            spacing="2",
                            align="start",
                            flex="2",
                            min_width="280px",
                        ...
                        rx.hstack(
                            _story_stat("Current fairness gap", AppState.twin_v2_fairness_gap_text, "Dispersion between cohort value outcomes."),
                            _story_stat("Intergenerational balance", AppState.twin_v2_intergen_balance_text, "Higher means generations are closer together."),
                            _story_stat("Stress pass rate", AppState.twin_v2_stress_pass_text, "Share of stress paths that remain acceptable."),
                            flex="3",
                            wrap="wrap",
                            spacing="2",
                        ),
                    ...
                    rx.text(
                        "This answers the jury question first: is the pension fair right now, and which generation is being pushed furthest away from actuarial parity?",
                        style={"color": PALETTE["muted"], "font_size": "11px", "line_height": "1.6"},
                    ),
                    spacing="3",
                    width="100%",
                ),
                _empty("Run the simulation to populate the fairness verdict."),
            ),
            subtitle="The headline verdict compresses the latest cohort MWR gap, intergenerational balance, and stress survival into a presenter-friendly answer.",
        ),
        rx.hstack(
            _panel(
                ...
                rx.cond(
                    ...
                    rx.vstack(
                        rx.recharts.bar_chart(
                            rx.recharts.bar(data_key="mwr_balanced", name="Balanced", stack_id="mwr", fill=PALETTE["good"]),
                            rx.recharts.bar(data_key="mwr_watch", name="Watch", stack_id="mwr", fill=PALETTE["warn"]),
                            rx.recharts.bar(data_key="mwr_stress", name="Stress", stack_id="mwr", fill=PALETTE["bad"]),
                            rx.recharts.x_axis(data_key="cohort_label", stroke=PALETTE["muted"]),
                            rx.recharts.y_axis(stroke=PALETTE["muted"], domain=[0, 1.6]),
                            rx.recharts.reference_line(y=1, label="actuarial parity", stroke=PALETTE["muted"], stroke_dasharray="4 4"),
                            rx.recharts.cartesian_grid(stroke=PALETTE["edge"]),
                            rx.recharts.legend(),
                            rx.recharts.graphing_tooltip(),
                            data=AppState.twin_v2_focus_cohort_rows,
                        ...
                        rx.text(
                            "MWR = EPV benefits / EPV contributions. Around 1.00 means actuarially fair value.",
                            style={"color": PALETTE["text"], "font_size": "11px", "line_height": "1.55"},
                        ),
                        rx.text(AppState.twin_v2_cohort_focus_note, style={"color": PALETTE["muted"], "font_size": "10px"}),
                        spacing="2",
                    ...
                    _empty("No cohort fairness snapshot yet."),
                ),
                subtitle="Representative latest-year cohorts. The reference line is actuarial parity: value received equals value contributed.",
            ),
            _panel(
                "Worst-hit cohorts",
                rx.cond(
                    AppState.twin_v2_worst_cohort_rows.length() > 0,
                    rx.box(
                        simple_table(
                            [
                                ("cohort", "Cohort"),
                                ("mwr_display", "MWR"),
                                ("stress_load_display", "Stress load"),
                                ("reason", "Reason"),
                                ("status", "Status"),
                            ],
                            AppState.twin_v2_worst_cohort_rows,
                        ),
                        style={"overflow_x": "auto"},
                    ),
                    _empty("No worst-hit cohort rows yet."),
                ),
                subtitle="Sorted by lowest latest-year MWR first, then stress load. This is the quick answer to who is harmed.",
            ),
            spacing="3",
            width="100%",
            align="stretch",
        ...
        _panel(
            ...
            rx.vstack(
                rx.hstack(
                    rx.vstack(
                        rx.text("Would governance be triggered?", style={"color": PALETTE["muted"], "font_size": "10px", "letter_spacing": "0.08em", "text_transform": "uppercase"}),
                        rx.text(AppState.twin_v2_governance_trigger_status, style={"color": PALETTE["text"], "font_size": "20px", "font_weight": "700"}),
                        rx.text(AppState.twin_v2_governance_trigger_text, style={"color": PALETTE["text"], "font_size": "12px", "line_height": "1.65"}),
                        spacing="2",
                        align="start",
                    ),
                    rx.spacer(),
                    rx.box(
                        rx.text("On-chain mapping", style={"color": PALETTE["muted"], "font_size": "10px", "letter_spacing": "0.08em", "text_transform": "uppercase"}),
                        rx.text("FairnessGate.submitAndEvaluate", style={"color": PALETTE["accent"], "font_size": "13px", "font_weight": "700", "font_family": "ui-monospace, SFMono-Regular, Menlo, monospace"}),
                        rx.text("The contract records PASS / FAIL against the fairness corridor.", style={"color": PALETTE["muted"], "font_size": "11px", "margin_top": "6px"}),
                        style={**CARD_STYLE, "min_width": "270px", "box_shadow": "none"},
                    ),
                    width="100%",
                    align="stretch",
                    spacing="3",
                ...
                rx.cond(
                    ...
                    rx.text("No unfair reform proposal has been triggered in this run.", style={"color": PALETTE["muted"], "font_size": "11px"}),
                ),
                spacing="3",
                width="100%",
            ),
            subtitle="The governance trigger explains what happens when the fairness story becomes a protocol decision.",
        ),
        _panel(
            "Advanced fairness diagnostics",
            rx.accordion.root(
                rx.accordion.item(
                    header=rx.text("Show abstract time-series diagnostics", style={"color": PALETTE["text"], "font_weight": "600", "font_size": "12px"}),
                    content=rx.vstack(
                        rx.hstack(
                            rx.text("View", style={"color": PALETTE["muted"], "font_size": "11px"}),
                            rx.select(
            ...
            subtitle="Kept for actuaries and debugging; the presenter view above answers the jury question first.",
        ),
        width="100%",
        spacing="0",
    )
...
def _execution_cost_tab() -> rx.Component:
    return rx.vstack(
        ...
        _panel(
            "Main cost driver",
            rx.cond(
                AppState.twin_v2_gas_driver_rows.length() > 0,
                rx.vstack(
                    rx.hstack(
                        rx.vstack(
                            rx.text("Main cost driver", style={"color": PALETTE["muted"], "font_size": "10px", "letter_spacing": "0.08em", "text_transform": "uppercase"}),
                            rx.text(AppState.twin_v2_gas_main_driver, style={"color": PALETTE["text"], "font_size": "28px", "font_weight": "800", "line_height": "1.1"}),
                            rx.text("Frequent member-level posting dominates gas cost when member cashflows are the largest driver.", style={"color": PALETTE["text"], "font_size": "12px", "line_height": "1.6"}),
                            spacing="2",
                            align="start",
                            flex="2",
                            min_width="260px",
                        ...
                        rx.hstack(
                            _story_stat("Share of total cost", AppState.twin_v2_gas_main_driver_share_fmt, "How much of the simulated bill comes from the largest action type"),
                            _story_stat("Driver cost", AppState.twin_v2_gas_main_driver_cost_fmt, "Cumulative cost from the largest action type"),
                            _story_stat("Recommendation", AppState.twin_v2_gas_recommendation_label, "Architecture choice implied by the cost driver"),
                            flex="3",
                            wrap="wrap",
                            spacing="2",
                        ),
                        rx.match(
                            AppState.twin_v2_gas_architecture_pill,
                            ("bad", pill(AppState.twin_v2_gas_architecture_status, "bad")),
                            ("warn", pill(AppState.twin_v2_gas_architecture_status, "warn")),
                            ("good", pill(AppState.twin_v2_gas_architecture_status, "good")),
                            pill("NO RUN", "muted"),
                        ),
                        width="100%",
                        spacing="4",
                ...
                _empty("Run the Twin to identify the main execution-cost driver."),
            ),
            subtitle="The key architecture question is whether the bill is audit-critical publication or frequent member-level execution.",
        ),
        _panel(
            "Architecture recommendation",
            rx.text(
                "If cost is mostly governance, oracle, proof, or reserve actions, Ethereum mainnet may be acceptable because those actions are infrequent and audit-critical. If cost is mostly member cashflows, production should Batch member cashflows, publish commitments, or move member-level execution to an L2. Aequitas should not blindly post every tiny member cashflow on Ethereum mainnet.",
                style={"color": PALETTE["text"], "font_size": "12px", "line_height": "1.7"},
            ),
            subtitle="Plain-English interpretation for the jury before the technical tables.",
        ),
        _panel(
        ...
        rx.hstack(
            ...
            _panel(
                ...
                rx.cond(
                    AppState.twin_v2_gas_driver_rows.length() > 0,
                    rx.recharts.bar_chart(
                        rx.recharts.bar(data_key="share_of_cost_pct", name="Share of total cost (%)", fill=PALETTE["warn"]),
                        rx.recharts.x_axis(data_key="action_type", stroke=PALETTE["muted"]),
                        rx.recharts.y_axis(stroke=PALETTE["muted"]),
                        rx.recharts.cartesian_grid(stroke=PALETTE["edge"]),
                        rx.recharts.legend(),
                        rx.recharts.graphing_tooltip(),
                        data=AppState.twin_v2_gas_driver_rows,
                        width="100%",
                        height=260,
                    ),
                    _empty("Gas-driving action types appear after the first run."),
                ),
                subtitle="Whole-run share by action type. This avoids hiding smaller categories under repeated annual member-cashflow bars.",
            ),
            spacing="3",
            width="100%",
            align="stretch",
        ...
        _panel(
            "Execution strategy by action type",
            rx.cond(
                AppState.twin_v2_gas_driver_rows.length() > 0,
                rx.box(
                    simple_table(
                        [
                            ...
                            ("share_label", "Share of cost"),
                            ("example_contract_mapping", "Example contract mapping"),
                            ("recommended_execution_strategy", "Recommended execution strategy"),
                        ],
                        AppState.twin_v2_gas_driver_rows,
                    ),
                    style={"overflow_x": "auto"},
                ),
                _empty("Execution strategy rows appear after the first run."),
            ),
            subtitle="The important output is not the exact fee quote; it is which architecture each action type deserves.",
        ),
        rx.hstack(
            _panel(
                "Preset comparison",
        ...
        _panel(
            ...
            rx.accordion.root(
                rx.accordion.item(
                    header=rx.text("Show detailed annual stacked breakdown", style={"color": PALETTE["text"], "font_weight": "600", "font_size": "12px"}),
                    content=rx.cond(
                        AppState.twin_v2_gas_annual_rows.length() > 0,
                        rx.recharts.bar_chart(
                        ...
                        _empty("Run the Twin to populate the annual stacked breakdown."),
                    ),
                    value="stacked",
                ),
                collapsible=True,
            ...
            subtitle="Kept for technical review only; the decision view above is the presenter view.",
        ),
        width="100%",
        spacing="0",
    )
